...
void readSensor_com(SckBase* base, String parameters)
{
	...
	else if (base->getReading(sensorToRead)) sprintf(base->outBuff, "%s: %s %s", base->sensors[sensorToRead].title, base->sensors[sensorToRead].reading.c_str(), base->sensors[sensorToRead].unit);
...
void monitorSensor_com(SckBase* base, String parameters)
{
	...
	while (!SerialUSB.available()) {
		...
		lastMillis = millis();
		for (uint8_t i=0; i<index; i++) {
			if (base->getReading(sensorsToMonitor[i])) sprintf(base->outBuff, "%s%s", base->outBuff, base->sensors[sensorsToMonitor[i]].reading.c_str());
